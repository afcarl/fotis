...
"""
def create_batches(path_to_structured_folder_tree, path_to_results, num_cases_per_batch, nr_of_images, image_size=32):
    ...
    batch = {"data": numpy.empty((real_batch_size, meta["num_vis"]),  dtype="uint8"),
             "labels": [],
             "batch_label": "",
             "filenames": []}
    batch_data_index = 0
    ...
    while folder_tree_dictionary:
        ...
        if batch_data_index == real_batch_size:
            # dump batch
            batch["batch_label"] = "Batch number %d" % batch_number
            batch_number += 1
            pickle(batch_data_index, join(path_to_results, batch["batch_label"].replace(" ", "_")))
